#!/usr/bin/env python3
"""
Download The Kaitchup archive - v5 (enhanced v4 with better organization)

This is an enhanced version of v4 with:
- Better filename organization (section prefix)
...
For full archive with auth, use v4 or v5 with SUBSTACK_SID cookie.

Usage:
  # Full archive with authentication (RECOMMENDED)
  export SUBSTACK_SID="YOUR_COOKIE"
  python download_kaitchup_v5.py

  # Public posts only (v3)
...
def download_post(post_data, output_dir, section="Archive"):
    """
    ...
    Args:
        ...
        section: Section name (Archive, Tutorials, Notes, AI Notebooks)
    ...
    section_prefix = section.replace(" ", "_").lower()
    filename = f"{section_prefix}_{timestamp}_{safe_title}.md"
    ...
    print(f"    [{section}] Saved: {filename}")
    return output_path
...
def main():
    ...
    print("\n" + "="*60)
    print("Fetching archive from Substack API...")
    print("="*60)
    ...
    for i, post in enumerate(all_posts):
        ...
        try:
            ...
            # Download to markdown with section prefix
            download_post(full_post, output_dir, "Archive")
            downloaded += 1
